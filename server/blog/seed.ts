...
type BlogSeedLogger = Pick<Console, "log" | "warn" | "error">;

type SeedBlogOptions = {
  logger?: BlogSeedLogger;
};
...
async function getOrCreateCategory(language: "en" | "es"): Promise<BlogCategory> {
  ...
  throw new Error(`Unable to create or find blog category ${language}/${category.slug}`);
}
...
async function getOrCreateTag(name: string, slug: string, language: "en" | "es"): Promise<BlogTag> {
  ...
  throw new Error(`Unable to create or find blog tag ${language}/${slug}`);
}

async function getOrCreatePost(values: typeof blogPosts.$inferInsert): Promise<BlogPost> {
  const language = values.language || "en";

  const [existing] = await db
    .select()
    .from(blogPosts)
    .where(and(eq(blogPosts.language, language), eq(blogPosts.slug, values.slug)))
    .limit(1);
  ...
  throw new Error(`Unable to create or find blog post ${language}/${values.slug}`);
}
...
<p>This article is educational and is not a substitute for emergency care or individualized medical advice. If you are in immediate danger or thinking about harming yourself, call 911 or go to the nearest emergency room.</p>`,
  ...
  const esPost = await getOrCreatePost({

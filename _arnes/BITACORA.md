...
**Qué se hizo:** Se corrigieron los dos P2 vigentes del PR #3. Cada ejecución del verificador usa ahora una dirección etiquetada y única, limpia filas antiguas del correo configurado y borra por correo además de por id para cubrir respuestas perdidas después de persistir.
**Decisiones:** El cambio queda aislado del corte de dominio y no altera la ruta pública de contacto. `VERIFY_CONTACT_EMAIL` conserva su función como base de entrega; la etiqueta `workflow-*` evita compartir el límite de tres envíos por hora entre ejecuciones.
**Pendientes/bugs:** Validar el workflow vivo en el preview inmutable del PR antes de resolver los dos hilos heredados del PR #3. El warning de `caniuse-lite` y las vulnerabilidades informadas por `npm audit` no se modificaron.
**Archivos tocados:** `scripts/verify-live-contact.ts`, `tests/contact-verification-workflow.test.mjs`, `_arnes/BITACORA.md`.
**Evidencia:** prueba enfocada PASS 3/3; `npm test` PASS 58/58; `npm run check` PASS; `npm run db:verify` PASS con 2 migraciones, 95 statements, 18 tablas, 20 foreign keys, ordered tags y contact insert; `npm run build` PASS con 89/89 páginas; ejecución controlada contra `healing-minds-psychiatry-nextjs.vercel.app` PASS 200, persistencia confirmada y limpieza confirmada. El preview inmutable del PR quedó detrás de Vercel Authentication y devolvió 401 en el borde antes de ejecutar la ruta.

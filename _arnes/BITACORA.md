...
**Qué se hizo:** Las preferencias de cookies usan ahora un estado de modal independiente de la franja inicial. Aceptar y rechazar cierran la interfaz; desde el pie, escritorio y móvil abren directamente el panel; X y Cancel descartan borradores; Guardar persiste la selección. El tracking delegado de teléfono solo considera lead el número oficial de la clínica. El auditor de Preview limita OIDC a un host `*.vercel.app` validado antes de usar el token. TikTok bloquea primero los eventos propios, revoca data sharing, desactiva cookies y conserva el marker de exclusión mientras elimina identificadores.
**Decisiones:** El marker `_tt_enable_cookie=0` no se trata como identificador publicitario ni se borra, porque el SDK lo usa para recordar el opt-out. La recreación tardía no se tapa ampliando el polling de producción: el auditor muestrea cada 500 ms durante 30 s y el sweep de la aplicación sigue acotado a seis segundos. Al reaceptar, TikTok ejecuta `enableCookie`, después `grantConsent` y solo entonces reabre el gate local y emite una vista deduplicada.
**Pendientes/bugs:** Publicar el PR, ejecutar el auditor de 30 s y el E2E completo sobre su Preview y SHA exactos, clasificar Code Review y obtener juez GO. Queda una decisión separada antes del cierre total: TikTok prohíbe transmitir datos de salud y el código heredado todavía emite rutas/eventos de condiciones y tratamientos; también hay que decidir si se retira definitivamente el build Vite legado de Replit.
...
**Evidencia:** TypeScript PASS, suite 100/100 PASS, build Next 89/89 PASS, guards de pageview PASS, pruebas browser 4/4 PASS, contrato focalizado del auditor 6/6 PASS y E2E local 12/12 PASS en escritorio y móvil. La prueba E2E incluye Manage, Reject, Accept, X, Cancel, Guardar, persistencia tras reload y descarte de toggles no guardados.

...
**Qué se hizo:** Se reprodujo que un solo clic cambiaba URL y título, pero dejaba el contenido anterior y disparaba `removeChild` en React. El layout marca ahora a Next como dueño único de los metadatos y el helper SEO legado no toca el `<head>` en ese runtime. Se añadió Playwright Test con recorridos EN/ES en desktop y móvil, incluyendo ida, atrás y adelante, valores SEO exactos y cero errores de página/consola; Quality ejecuta el E2E después del build. El gate desplegado exige URL HTTPS y SHA completo, y compara ese SHA con el metadato real del build de Vercel.

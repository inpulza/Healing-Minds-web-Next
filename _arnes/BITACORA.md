...
## 2026-08-20 Codex - HM Web 02 datos estructurados, AEO/GEO y SEO local
**Qué se hizo:** Se demostró que producción tenía JSON-LD solo en 1 de 80 URLs y que el generador completo de Express quedó desconectado del App Router. Se creó un grafo SSR por ruta, perfil canónico de negocio/persona, cobertura de servicios, ciudades, FAQs, blog y artículos, robots/llms y contenido esencial en HTML inicial. La auditoría independiente de las 20 rutas de ubicación corrigió rutas falsas, tiempos duplicados, wording de sucursal y claims operativos/telehealth sin respaldo.
**Decisiones:** Una única sede física Naples; nueve ciudades como Service/areaServed. NAP desde GBP/NPPES y un perfil compartido. Sin fax, aceptación de pacientes, rating, precios, board certification ni claims clínicos no confirmados. La modalidad se confirma caso por caso; telesalud depende de ubicación/licencia; coordinación de registros requiere consentimiento.
**Pendientes/bugs:** Crear PR draft, verificar Preview del SHA exacto con 80 URLs/Neon, reintentar Schema.org/Rich Results, clasificar Code Review, integrar por squash y repetir auditoría completa en Production. Confirmaciones operativas pendientes: fax correcto y documentación de board certification si existe.
**Archivos tocados:** App Router SEO/robots/llms, perfil compartido, componentes SSR FAQ/contacto, contenido local EN/ES, rutas legacy desacopladas, pruebas unitarias/E2E, playbook, decisiones, evidencia y bitácora.
**Evidencia local hasta este registro:** TypeScript PASS; 151/151 unitarios/contrato PASS; build 87 páginas y budgets 669.5/750 KiB + 780.0/850 KiB PASS; Playwright 98 PASS/22 skips deliberados/0 fallos en 120 casos desktop+móvil; crawl local 77/77 con 200, un grafo por URL y 0 JSON inválidos; DOM 14/14, 0 páginas con error de consola; juez independiente PASS. Schema.org devolvió HTTP 405 al reintento y queda como gate externo de Preview, no como PASS.

## 2026-08-20 Codex - Preview PR #31 y robustez del smoke de headings/mapa
**Qué se hizo:** El Preview del SHA `5c9a46e167d21f6e806dad3877b689374f87a39f` recorrió el sitemap real de 80 URLs y la suite desplegada completa. GitHub Quality detectó una carrera al desplazar un boundary recién hidratado; la reproducción local mostró además una espera circular al cerrar el request de Maps retenido por el propio test. Se añadieron reintentos acotados solo para el detach transitorio y cleanup `ignoreErrors` para liberar el handler.
**Decisiones:** No se desactiva ningún gate ni se ignora el fallo. El test conserva todas las aserciones de headings, mapa y fallback, añade trazas por etapa y debe volver a pasar en GitHub sobre el nuevo SHA.
**Pendientes/bugs:** Commit/push del ajuste, nuevo Preview exacto, Quality verde, validadores externos y clasificación completa de Code Review antes del merge.
**Archivos tocados:** smoke de jerarquía/mapa, evidencia y bitácora.
**Evidencia:** deployment `dpl_7y5JwMJPeC2KEpynoKqYD6dRwQxT` Ready; crawl 80/80 status 200, 80/80 con un JSON-LD y 0 inválidos; DOM 14/14 y 0 errores de consola; E2E desplegado 98 PASS/22 skips/0 fallos; smoke reparado PASS local en 18.5 s.

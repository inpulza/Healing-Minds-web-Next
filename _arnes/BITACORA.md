...
**Qué se hizo:** Las notas CodeX posteriores detectaron dos bordes válidos. Google conserva ahora en memoria la decisión efectiva del documento, de modo que una retirada sigue cerrando pageviews y leads aunque falle la escritura y quede un grant antiguo en localStorage. TikTok reafirma `enableCookie` y `grantConsent` en cada documento con marketing permitido, antes de `page`; si la restauración falla después de habilitar cookies, revoca, deshabilita, mantiene cerradas las compuertas y continúa limpiando identificadores. El banner espera a leer la preferencia local antes de mostrarse, eliminando el flash intermitente observado en historial. La última ronda de jueces añadió dos fallos de excepción válidos: un error de `gtag` ya no puede saltarse la limpieza local, y un navegador que bloquea simultáneamente `getItem` y `removeItem` sigue hidratando la UI con estado denegado. La revisión CodeX del siguiente SHA añadió sincronización cross-tab y recuperación del pageview TikTok: una retirada en otra pestaña se rebroadcast a todos los proveedores, un grant obsoleto se contrasta con el valor vigente antes de aplicarlo, y un retry después de fallo emite la ruta pendiente antes de marcarla.
**Decisiones:** La restauración del proveedor se trata como una operación transaccional: la limpieza solo se detiene tras completar ambos controles. Un permiso persistido se reafirma en cada documento porque una recarga rápida puede ocurrir antes de que el SDK asíncrono consuma la cola anterior. Las operaciones y limpiezas Google quedan aisladas para que una excepción no cancele la siguiente salvaguarda. Los tests de navegador no usan retries para ocultar inestabilidad.
**Pendientes/bugs:** Publicar este árbol en un SHA nuevo, exigir Quality y Code Review exactos, ejecutar 24/24 E2E y el auditor real de 30 segundos contra la Preview inmutable, y clasificar/resolver todos los hilos vigentes. No fusionar hasta decidir el riesgo contractual separado del TikTok Pixel sitewide en páginas clínicas.
...
**Evidencia:** Tres jueces independientes GO sin P0-P2 tras resolver cada NO-GO. TypeScript PASS, suite 102/102 PASS, build 89/89 PASS, guard de analytics PASS, matriz de consentimiento repetida 30/30 sin retries, casos focalizados y E2E completo limpio 24/24 PASS. La cobertura fuerza un fallo de `grantConsent` después de `enableCookie`, exige rollback `revokeConsent -> disableCookie`, navega y recupera exactamente un `page`, verifica limpieza continua, bloquea navegación/eventos Google tras un write fallido o una retirada en otra pestaña, rechaza un storage event obsoleto, elimina `_ga` y `_gcl_*` aunque `gtag` lance, prueba Storage `SecurityError` y comprueba `enableCookie -> grantConsent -> page` tanto en reaceptación como después de una recarga inmediata. El P3 de polling realiza ahora 20 esperas y una lectura final real. Una pasada 21/22 quedó invalidada porque un juez reconstruyó `.next` y otro compartió `test-results` mientras el servidor E2E los usaba; con procesos paralelos cerrados, las builds limpias pasaron 22/22 y después 24/24.
